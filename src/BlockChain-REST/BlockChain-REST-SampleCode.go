...
func main() {
	...
	answer = InvokeChaincode("loadSampleCars", "", 1)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")

	answer = InvokeChaincode("loadMaintenanceSample", "", 2)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")

	answer = QueryChaincode("getCar", `"0001"`, 3)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")

	answer = QueryChaincode("listCars", "", 4)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")

	answer = QueryChaincode("getCarMaintenceList", `"0001"`, 5)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")
	...
	answer = InvokeChaincode("addMaintenance", `"0010", "Garage X", "Maintenance", "23-03-2017", "1234", "First checkup"`, 8)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")

	answer = QueryChaincode("getCarMaintenceList", `"0010"`, 9)
	fmt.Println(answer)
	time.Sleep(5 * time.Second)
	fmt.Println("\n")
}
